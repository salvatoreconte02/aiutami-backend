"""Test endpoint + modello LostAtSeaIndividualRanking.

Suddiviso in classi:
- ModelTests: schema, constraint unicità.
- ViewTests: GET/PUT/POST submit/POST finalize-if-expired.
"""
...
class LostAtSeaIndividualRankingGetPutTests(TestCase):
    def setUp(self) -> None:
        ...
        self.url = f"/api/tasks/lost-at-sea/sessions/{self.session.id}/individual-ranking/"
    ...
    def test_put_creates_row(self) -> None:
        resp = self.client.put(
            self.url, data={"ranked_items": list(LOST_AT_SEA_ITEMS)}, format="json",
        )
        self.assertIn(resp.status_code, (200, 201), resp.json())
        from apps.tasks.lost_at_sea.models import LostAtSeaIndividualRanking
        r = LostAtSeaIndividualRanking.objects.get(session=self.session, participant=self.participants[0])
        self.assertEqual(r.ranked_items, list(LOST_AT_SEA_ITEMS))
        self.assertFalse(r.is_submitted)

    def test_put_invalid_length(self) -> None:
        resp = self.client.put(
            self.url, data={"ranked_items": list(LOST_AT_SEA_ITEMS)[:14]}, format="json",
        )
        self.assertEqual(resp.status_code, 400)

    def test_put_invalid_items(self) -> None:
        bad = list(LOST_AT_SEA_ITEMS)
    ...
    def test_put_after_submit_blocked(self) -> None:
        ...
        LostAtSeaIndividualRanking.objects.create(
            ...
            ranked_items=list(LOST_AT_SEA_ITEMS), is_submitted=True,
        )
        resp = self.client.put(
            self.url, data={"ranked_items": list(LOST_AT_SEA_ITEMS)}, format="json",
        )
...
class LostAtSeaIndividualRankingSubmitTests(TestCase):
    def setUp(self) -> None:
        self.session, self.participants = _make_session()
        _put_session_in_individual_ranking(self.session)
        self.client = APIClient()
        self.url = f"/api/tasks/lost-at-sea/sessions/{self.session.id}/individual-ranking/submit/"
        self.put_url = f"/api/tasks/lost-at-sea/sessions/{self.session.id}/individual-ranking/"
    ...
    def test_submit_marks_is_submitted(self) -> None:
        ...
        c.put(self.put_url, data={"ranked_items": list(LOST_AT_SEA_ITEMS)}, format="json")
...
class LostAtSeaIndividualRankingFinalizeIfExpiredTests(TestCase):
    def setUp(self) -> None:
        self.session, self.participants = _make_session()
        _put_session_in_individual_ranking(self.session)
        self.client = APIClient()
        self.client.force_authenticate(user=self.participants[0].user)
        self.url = f"/api/tasks/lost-at-sea/sessions/{self.session.id}/individual-ranking/finalize-if-expired/"
