...
\begin{enumerate}
    ...
    \item \textbf{Turn-Taking Cycle} --- Participants take turns speaking via push-to-talk. While one user speaks, others can reserve the next turn. The reserved user receives an 8-second priority window after the current speaker finishes.
    \item \textbf{Moderation Triggers} --- Throughout the discussion, the AI moderator monitors participation and intervenes when needed: prompting inactive users, addressing collective silence, or redirecting the conversation.
    \item \textbf{Ready to Conclude} --- In the Murder Mystery context, participants press ``Ready to Conclude'' when they believe they have identified the culprit. The moderator announces each declaration to the group.
    \item \textbf{Conclusion Phase} --- Triggered either when all participants declare readiness or when the 30-minute timer expires. The moderator delivers a vocal summary of the discussion.
    \item \textbf{Voting} --- Each participant votes for the suspect they believe is guilty. Results are revealed once all votes are cast.
    \item \textbf{Session Closure} --- The host closes the session. The system generates a final report.
